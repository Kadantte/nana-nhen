...
class NanaEmbeds {
  ...
  async getEmoji(id, m, msg) {
    ...
    const favoritFilter = (reaction, user) =>
      reaction.emoji.name === `💖` && user.id === msg.author.id;
    const deletes = m.createReactionCollector(deleteFilter, { time: 7.2e+6 });
    const forwards = m.createReactionCollector(forwardsFilter, { time: 7.2e+6 });
    const download = m.createReactionCollector(downloadFilter, { time: 7.2e+6 });
    const favorit = m.createReactionCollector(favoritFilter, { time: 7.2e+6 });
  ...
  async getRead(res, read, r, msg, pagination) {
    ...
    const forwardsTenFilter = (reaction, user) =>
      reaction.emoji.name === `⏩` && user.id === msg.author.id;
    const backwardsTen = r.createReactionCollector(backwardsTenFilter, { time: 7.2e+6 });
    const backwards = r.createReactionCollector(backwardsFilter, { time: 7.2e+6 });
    const deletes = r.createReactionCollector(deleteFilter, { time: 7.2e+6 });
    const forwards = r.createReactionCollector(forwardsFilter, { time: 7.2e+6  );
    const forwardsTen = r.createReactionCollector(forwardsTenFilter, { time: 7.2e+6 });
...
<<<<<<< HEAD
    let nhentURL = `${this.client.mangadl}/download/nhentai/${res.id}/?e=${
=======
    let nhentURL = `${this.client.mangadl}/${res.id}/?e=${
>>>>>>> 18228865d4222fd158a17ab74cde286c67996941
      type == "cbz" ? "cbz" : "zip"
    }`;
    const embed = new MessageEmbed()
