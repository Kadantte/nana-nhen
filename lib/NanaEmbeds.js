...
class NanaEmbeds {
    ...
    return new Promise(async (fulfill, reject) => {
      try {
        ...
        download.on("collect", d => {
          let nhentURL = `https://mangadl2.herokuapp.com/download/nhentai/${res.id}/zip`;
          const embed = new MessageEmbed()
        ...
        favorit.on("collect", async f => {
          let favorite = await this.client.favorite.getUserFavoritID(
            msg.author.id
          );
  ...
  getRead(res, read, r, msg, pagination) {
    return new Promise(async (fulfill, reject) => {
      try {
        let doujin = [];
        res.images.pages.forEach((page, i) => {
